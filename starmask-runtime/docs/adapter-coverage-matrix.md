# Starmask Runtime Shim Coverage Matrix

## Status

This note tracks coverage for the historical `v1` `starmask-runtime` adapter layer.

Repository status note: `crates/starmask-runtime` has been removed from the workspace, so this matrix
...
- `docs/adapter-real-environment-runbook.md`

## Adapter Automated Coverage

Historical local automated coverage lived in `crates/starmask-runtime/tests/` before that adapter was
removed:
...
These are not good fits for more `starmask-runtime`-only tests and should instead be covered by the
...
| --- | --- |
| MCP Inspector over stdio against a running `starmaskd` and `starmask-runtime` | validates real stdio wiring, process startup, and inspector interoperability |
