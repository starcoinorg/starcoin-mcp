...
This file adds stricter instructions for the `starmask-runtime` subproject.
...
- The workspace does not currently ship an in-tree stdio adapter boundary.
...
- Do not scatter SQL across host-adapter handlers or Native Messaging handlers.
...
- For each new `starmask-runtime` task branch, prefer a dedicated git worktree based on the latest `main`.
...
- After a `starmask-runtime` change reaches a verified milestone, commit it promptly.
