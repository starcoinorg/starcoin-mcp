...
8. `transaction watch until terminal confirmation or timeout`: initiator `MCP host`; chain snapshot or capability bounded polling through `watch_transaction`; failure codes `rpc_unavailable`, `transaction_not_found`, `rate_limited`; host retry `yes` with backoff while preserving `txn_hash`; degradation time budgets may be clamped but the effective values are returned. References: `starcoin-node-mcp-interface-design.md` watch tool and resource results; `deployment-model.md` runtime and backpressure rules; `testing-and-acceptance.md` resource and end-to-end acceptance.
