# Starcoin Node MCP Security Model

## Purpose

This document defines the security and safety model for `starcoin-node-mcp`.
...
- an external wallet-facing signer such as `starmask-mcp`

## Security Goal

The primary security goal is:

`starcoin-node-mcp` must let a local MCP host access chain state and prepare transaction payloads without creating false signing trust in the node endpoint or crossing the wallet boundary.

## Security Invariants

The following rules must remain true in every implementation phase:

1. `starcoin-node-mcp` never holds private keys.
2. `starcoin-node-mcp` never signs transactions.
...
- the wallet-facing system such as `starmask-mcp` and its extension
