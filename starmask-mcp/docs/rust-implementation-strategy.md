...
3. it reduces the amount of MCP transport and protocol boilerplate that `starmask-mcp` itself must own

Scope rule:

- use `rmcp` only for the MCP-facing `starmask-mcp` binary
...
`starmask-mcp` is a thin local adapter.
