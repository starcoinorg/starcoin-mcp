...
fn transaction_probe_requires_submission_and_dry_run() {
    ...
    assert_eq!(
        validate_transaction_probe(&missing_submit)
            ...
            .code,
        starcoin_node_types::SharedErrorCode::UnsupportedOperation
    );
}
...
fn head_lag_policy_fails_closed_above_threshold() {
    ...
    assert_eq!(
        enforce_transaction_head_lag(200, 100, Duration::from_secs(30))
            ...
            .code,
        starcoin_node_types::SharedErrorCode::RpcUnavailable
    );
}
...
fn resolve_expiration_rejects_past_requested_timestamps() {
    ...
    assert_eq!(
        error.code,
        starcoin_node_types::SharedErrorCode::TransactionExpired
    );
}
...
fn validate_signed_submission_rejects_chain_mismatch() {
    ...
    assert_eq!(
        error.code,
        starcoin_node_types::SharedErrorCode::InvalidChainContext
    );
}
...
fn submit_result_helpers_preserve_policy_shape() {
    ...
    assert_eq!(
        ...
        starcoin_node_types::SubmissionState::Accepted
    );
    ...
    assert_eq!(
        unknown.next_action,
        starcoin_node_types::SubmissionNextAction::ReconcileByTxnHash
    );
}
...
async fn resolve_sequence_number_degrades_when_txpool_sequence_hint_is_unavailable() {
    ...
    assert_eq!(source, starcoin_node_types::SequenceNumberSource::Onchain);
}
