# Starcoin Node Interface Design

## 1. Purpose

This document defines the interface design for `starcoin-node`, a chain-facing runtime and
host-facing tool surface for Starcoin.

The design assumptions are:

- the MCP host is Claude Code, Codex, or a similar local MCP-capable host
- `starcoin-node` may connect to a local node or a remote node
- `starcoin-node` does not hold private keys
- signing is delegated to a separate wallet-facing system such as `starmask-runtime`
...
6. Structured outputs must be optimized for host orchestration.
...
8. The first implementation should preserve explicit Rust boundaries among host transport, RPC adaptation, and core transaction logic.

## 3. Runtime Topology

```mermaid
flowchart LR
    H["MCP Host"] --> N["starcoin-node"]
...
`starcoin-node` is responsible for:

- normalizing Starcoin RPC into a smaller host tool surface
...
`starcoin-node` may internally use Starcoin JSON-RPC clients, but the MCP surface should remain stable even if the underlying RPC method set evolves.
...
The RPC surface classification and normalization rules for this layer are defined in `starcoin-node/docs/rpc-adapter-design.md`.
...
- `starcoin-node.prepare_*`
- optional `starcoin-node.simulate_raw_transaction`
- `starmask-runtime.wallet_request_sign_transaction`
- `starcoin-node.submit_signed_transaction`

## 10. Deployment Model

The canonical deployment and profile rules are defined in `starcoin-node/docs/deployment-model.md`.
