# Starcoin Node Deployment Model

## Purpose

This document defines the deployment and runtime model for `starcoin-node`, the chain-facing
Starcoin node integration layer.

Repository status note: the in-tree `starcoin-node-server` adapter has been removed. The
...
```mermaid
flowchart LR
    H["MCP Host"] --> M["starcoin-node (stdio)"]
...
- launches `starcoin-node`
- selects the intended workflow
- coordinates with `starmask-runtime` when signing is required
...
1. the MCP host launches `starcoin-node`
2. `starcoin-node` loads configuration
...
The first release assumes one endpoint per process. A single `starcoin-node` instance should not switch among multiple endpoints mid-session.
...
- `starcoin-node` may exit immediately
