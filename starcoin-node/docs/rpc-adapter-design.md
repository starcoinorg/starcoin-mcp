# Starcoin Node RPC Adapter Design

## Purpose

This document defines how `starcoin-node` should map the Starcoin RPC surface into a stable
host-facing tool interface.
...
out of the host-facing tool contract.

## Design Goals

The adapter layer should optimize for:

1. one stable task-oriented host surface
...
5. minimal leakage of Starcoin JSON-RPC details into host tools
...
- `mapper`
  - converts RPC-native views into stable host result shapes
...
- the CLI or any future host adapter should depend on typed adapter traits, not raw RPC method names
...
- `starcoin-node-server`
  - historical host-transport boundary; not currently shipped in-tree
- `starcoin-node-core`
...
The host tool surface remains version-neutral, but the backing Starcoin JSON-RPC surface does not.
